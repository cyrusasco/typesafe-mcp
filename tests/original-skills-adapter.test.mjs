...
test('Windows inspection preserves bounded OS startup paths but no tokens or custom execution hooks', () => {
  ...
  assert.equal(env.TEMP,'C:\\Temp'); assert.equal(env.USERPROFILE,'C:\\Users\\fixture');
  assert.equal(env.LOCALAPPDATA,'C:\\Users\\fixture\\AppData\\Local');
  assert.equal(env.TYPESAFE_API_KEY,undefined); assert.equal(env.NODE_OPTIONS,undefined); assert.equal(env.PATH,undefined);
});
